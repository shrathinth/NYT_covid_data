...
nytwhole = read_csv("T:/Venky/Data/Covid/NYT_covid_data/us-counties.csv") # loads nytimes 
...
for (i in 1:(length(dates))){ # iterates through all the distinct dates, creating 
  ...
  fullcumulative = merge(geocorr2022, nyt, by.x = "county", by.y = "fips", all.x = TRUE)
  fullcumulative$date = max(nyt$date)
  fullcumulative[is.na(fullcumulative)]=0
  fullcumulative$pop20 <- as.numeric(fullcumulative$pop20)
  ...
  NYCcumulative$countypop = sum(NYCcumulative$pop20)
  ...
  KCcumulative$countypop = sum(KCcumulative$pop20)
  ...
  fullcumulative4$hrrcaserate = fullcumulative4$countycaserate*(fullcumulative4$pop20/fullcumulative4$hrrpop) # multiples the rates by the allocation factors in the crosswalk
  fullcumulative4$hrrdeathrate = fullcumulative4$countydeathrate*(fullcumulative4$pop20/fullcumulative4$hrrpop)
  fullcumulative4$accuracy_index = (fullcumulative4$pop20/fullcumulative4$hrrpop)*as.numeric(fullcumulative4$afact)
  ...
  fullcumulative6 = distinct(fullcumulative5, hrr19, .keep_all = TRUE)
  fullcumulative7 = fullcumulative6 %>%
    select(hrr19, hrrpop, hrrcaserate, hrrdeathrate, hrrcaserate100k, hrrdeathrate100k, date, accuracy_index)
  ...
  fullcumulative8 = subset(fullcumulative7, hrr19 != "Hospital referral regions (2019)") # removes extra text rows 
...
write.csv(outcounty.file, file = "T:/Venky/Data/Covid/NYT_covid_data/CasesandDeathsbyCounty.csv")
write.csv(outhrr.file, file = "T:/Venky/Data/Covid/NYT_covid_data/CasesandDeathsbyHRR.csv")
